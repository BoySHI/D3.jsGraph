<template>
  <div>
    ...
    <div id="info"
      ...
      <p v-for="(item, key) in selectNodeData.data" :key="item">
        <span>{{ key }}</span>
        {{ item }}
      </p> -->
    ...
    threeRender () {
      ...
      this.Graph.height(750).width(1200)
        ...
        .nodeColor(node => {
          ...
          return node.data.color // 节点的颜色
        })
        // .nodeAutoColorBy('label')
        // 给节点添加文字
        // .nodeThreeObjectExtend(true)
        .nodeThreeObject(node => {
          const sprite = new SpriteText(node.data.name)
          ...
          sprite.color = '#333' // 节点上文字的颜色
          sprite.textHeight = 3 // 节点上文字的大小
          return sprite
        })
        .nodeThreeObjectExtend(true)
        .nodeLabel(node => `${node.label}: ${node.data.name}`) // 鼠标移动到节点上显示的节点的相关信息
        .nodeOpacity(0.9)// 节点的透明度
        // 节点事件绑定
        .onNodeHover(node => elm.style.cursor = node ? 'pointer' : null)
        .onNodeClick(node => {
          console.log(node)
          //设置#info h4样式的颜色为该节点的颜色，文本为该节点name
          // 这块是和鼠标移动上节点 右下显示节点信息的编辑小框
          this.$set(this.selectNodeData, 'id', node.id)
          this.$set(this.selectNodeData, 'name', node.data.name)
          // 获取节点类型对应的颜色
          // let i = 0
          // for (; i < this.labels.length; i++) {
          //   if (node.label === this.labels[i]) break
          // }
          this.$set(this.selectNodeData, 'color', '#eaeaea') //信息框的颜色
          this.$set(this.selectNodeData, 'data', node.data)
        })
        .nodeResolution([50])
        // 关系样式
        //.linkColor('red')
